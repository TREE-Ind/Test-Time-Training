...
@dataclass
class PrecompConfig:
    """Precomputation-specific configuration options."""
    
    episode_dir: Optional[str] = field(default=None, metadata={"help": "Directory containing input episodes"})
    video_length: int = field(default=18, metadata={"help": "Length of videos in seconds"})
    fps: int = field(default=16, metadata={"help": "Frames per second"})
    vae_tiling_window_unit: int = field(default=1, metadata={"help": "VAE temporal tiling window unit"})
    batch_size: int = field(default=1, metadata={"help": "Batch size for processing"})
    vae_weight_path: Optional[str] = field(default=None, metadata={"help": "Path to VAE weights"})
    output_dir: Optional[str] = field(default=None, metadata={"help": "Directory to save encodings"})
