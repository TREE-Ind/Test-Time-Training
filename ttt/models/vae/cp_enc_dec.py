...
from ttt.models.vae.utils import SafeConv3d as Conv3d
...
def nonlinearity(x, use_silu=False):
    if use_silu:
        return nn.SiLU()(x)
    else:
...
class ContextParallelResnetBlock3D(nn.Module):
    ...
    def forward(self, x, temb, zq=None, clear_fake_cp_cache=True):
        ...
        h = nonlinearity(h, self.use_silu)
        h = self.conv1(h, clear_cache=clear_fake_cp_cache)

        if temb is not None:
            h = h + self.temb_proj(nonlinearity(temb, self.use_silu))[:, :, None, None, None]
...
class ContextParallelEncoder3D(nn.Module):
    def __init__(
        ...
        **ignore_kwargs,
            ...
            for i_block in range(self.num_res_blocks):
                block.append(
                    ContextParallelResnetBlock3D(
                        ...
                        gather_norm=config.gather_norm,
        ...
        # end
        self.norm_out = Normalize(block_in, gather=config.gather_norm)

        self.conv_out = ContextParallelCausalConv3d(
            chan_in=block_in,
            chan_out=2 * config.z_channels if config.double_z else config.z_channels,
            kernel_size=3,
        )

    def forward(self, x, clear_fake_cp_cache=True, **kwargs):
        # timestep embedding
        temb = None
...
class ContextParallelDecoder3D(nn.Module):
    def __init__(
        ...
        **ignorekwargs,
        ...
        if zq_ch is None:
            zq_ch = config.z_channels

        # compute in_ch_mult, block_in and curr_res at lowest res
        in_ch_mult = (1,) + tuple(config.ch_mult)
        block_in = self.ch * config.ch_mult[self.num_resolutions - 1]
        curr_res = self.resolution // 2 ** (self.num_resolutions - 1)
        self.z_shape = (1, config.z_channels, curr_res, curr_res)
        print("Working with z of shape {} = {} dimensions.".format(self.z_shape, np.prod(self.z_shape)))

        self.conv_in = ContextParallelCausalConv3d(
            chan_in=config.z_channels,
            chan_out=block_in,
            kernel_size=3,
        )
        ...
        self.norm_out = Normalize3D(block_in, zq_ch, add_conv=add_conv, gather=config.gather_norm)

        self.conv_out = ContextParallelCausalConv3d(
            chan_in=block_in,
            chan_out=config.out_ch,
            kernel_size=3,
        )
