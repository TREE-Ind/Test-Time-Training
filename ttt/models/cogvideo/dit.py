...
from ttt.models.cogvideo.utils import (Rotary3DPositionEmbedding,
                                       ...
                                       place_into, shard_tensor,
                                       timestep_embedding, to_local, unpatchify)
...
class SeqModelingBlock(nn.Module):
    ...
    def _attn_forward(self, vid_emb: torch.Tensor, text_emb: torch.Tensor, seq_metadata: SequenceMetadata):
        ...
        cur_emb = torch.cat([text_emb, vid_emb], dim=1)
        ...
        def do_attention(cur_emb):
            ...
            # Assuming rotary embeddings should be applied to the video part
    ...
    def _ssm_forward(self, emb: torch.Tensor, seq_metadata: SequenceMetadata):
        ...
        # Forward pass
        forward_residual = emb
        emb = forward_ssm(emb, seq_metadata)
        emb = self._gate(
            self.forward_ssm_gating_text, self.forward_ssm_gating_video, forward_residual, emb, text_length
        )

        # Backward pass
        backward_residual = emb
        emb = torch.flip(emb, dims=[1])
        emb = reverse_ssm(emb, seq_metadata)
        emb = torch.flip(emb, dims=[1])
        emb = self._gate(
            self.backward_ssm_gating_text, self.backward_ssm_gating_video, backward_residual, emb, text_length
        )
    ...
    def forward(self, vid_emb: torch.Tensor, text_emb: torch.Tensor, seq_metadata: SequenceMetadata):
        ...
        # Pad sequence to be a multiple of mini_batch_size
        original_len = output.shape[1]
        padding_size = (self.config.mini_batch_size - original_len % self.config.mini_batch_size) % self.config.mini_batch_size
        if padding_size > 0:
            padding = torch.zeros(output.shape[0], padding_size, output.shape[2], device=output.device, dtype=output.dtype)
        ...
        #     local_output = self.ssm.ttt.undo_interleave(to_local(output), seq_metadata)
        #     output = place_into(local_output, output)

        text_length = seq_metadata.seq_text_length
        vid_seq_output = output[:, text_length:]
        text_seq_output = output[:, :text_length]

        return vid_seq_output, text_seq_output
