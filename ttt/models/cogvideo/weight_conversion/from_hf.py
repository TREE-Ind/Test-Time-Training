...
def main(final_save_path, ssm_layer, path_to_weights):
    ...
    file_list = [
        os.path.join(path_to_weights, f)
        for f in os.listdir(path_to_weights)
        if f.endswith(".safetensors")
    ]
    print(f"Found {len(file_list)} safetensors files in {path_to_weights}:")
    for f in file_list:
        print(f"  - {f}")
    ...
    for filename in file_list:
        ...
        if not os.path.exists(filename):
            print(f"    [ERROR] File not found at this path. Skipping.")
            continue
        
        file_size = os.path.getsize(filename)
        print(f"    [INFO] File exists. Size: {file_size} bytes.")
        if file_size < 1024: # Safetensors headers are usually small, but the file shouldn't be tiny
             print(f"   [WARNING] File size is very small. It might be corrupted or empty.")

        try:
            with safe_open(filename, framework="pt", device="cpu") as f:
                for key in f.keys():
                    all_tensors[key] = f.get_tensor(key)
                print(f"    [SUCCESS] Successfully loaded tensors from {filename}")
        except Exception as e:
            print(f"    [ERROR] Failed to open {filename} with safetensors.")
            print(f"    Exception: {e}")
