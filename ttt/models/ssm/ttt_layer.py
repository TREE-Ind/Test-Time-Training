...
class TTTBase(nn.Module):
    ...
    def process_input(self, hidden_states: torch.Tensor, freqs_cis: torch.Tensor, seq_metadata: SequenceMetadata):
        ...
        XQ = XQ.view(B, L, self.num_heads, self.head_dim)
        XK = XK.view(B, L, self.num_heads, self.head_dim)
        ...
        # Pad freqs_cis to match the padded sequence length of the video
        padded_video_len = XQ_video.shape[1]
        if freqs_cis.shape[0] < padded_video_len:
            padding_size = padded_video_len - freqs_cis.shape[0]
            padding = torch.zeros(padding_size, freqs_cis.shape[1], device=freqs_cis.device, dtype=freqs_cis.dtype)
        ...
        X = hidden_states
        X, XQ, XK, XV = self.reshape_to_mini_batch(X, XQ, XK, XV)

        ttt_lr_eta = self.get_eta(X)
        eta = 1 / self.mini_batch_size * ttt_lr_eta.repeat(1, 1, 1, self.mini_batch_size, 1)

        # if seq_metadata.is_multiscene:
        #     XQ, XK, XV, eta = map(
        #         lambda x: self.interleave(x, seq_metadata),
        #         (XQ, XK, XV, eta),
        #     )
...
class TTTLinear(TTTBase):
    def __init__(self, config: ModelConfig, use_kernel: bool = True):
        ...
        self.W1 = nn.Parameter(torch.empty(self.num_heads, self.head_dim, self.head_dim))
        self.b1 = nn.Parameter(torch.empty(self.num_heads, 1, self.head_dim))
    ...
    def forward(
        ...
        assert hidden_states.size(1) % self.config.mini_batch_size == 0, "Sequence len must be multiple of mini batch size."
        XQ, XK, XV, eta = self.process_input(hidden_states, freqs_cis, seq_metadata)

        B = XV.shape[0]
        L = hidden_states.shape[1]
        num_mini_batch = XV.shape[2]
        ...
        if self.use_kernel:
            ...
            XQW_batch = ttt_linear(
                ...
                to_local(XV),
                to_local(eta),
                to_local(self.ttt_norm_weight),
                to_local(self.ttt_norm_bias),
                to_local(W1_states),
                to_local(b1_states),
                checkpoint_group_size,
            )
        ...
        hidden_states = place_into(XQW_batch, hidden_states)

        hidden_states = self.post_norm(hidden_states)
        hidden_states = self.wo(hidden_states)
        return full_tensor(hidden_states)
...
class TTTMLP(TTTBase):
    def __init__(self, config: ModelConfig, use_kernel: bool = True):
        super().__init__(config)
        self.use_kernel = use_kernel
        # Init weights for token mixing
        self.W1 = nn.Parameter(torch.empty(self.num_heads, self.head_dim, self.head_dim))
        self.b1 = nn.Parameter(torch.empty(self.num_heads, 1, self.head_dim))

        self.W2 = nn.Parameter(torch.empty(self.num_heads, self.head_dim, self.head_dim))
        self.b2 = nn.Parameter(torch.empty(self.num_heads, 1, self.head_dim))
